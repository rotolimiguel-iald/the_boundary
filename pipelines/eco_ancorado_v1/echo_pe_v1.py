...
DT_MAX = 0.1; A_LO, A_HI = -3.0, 3.0; PRIOR_WIDEN = 2.0; CHI_DEFAULT = 0.2
NLIVE, WALKS, DLOGZ = 300, 30, 0.3
FAMILIES = {'A': 'IMRPhenomD', 'B': 'SEOBNRv4'}; INJ_T0 = -12.0
...
def make_priors(ev, dets):
    ...
    z1 = 1 + ev['z']
    pr['m1'] = bilby.core.prior.Uniform(max(1.0, (ev['m1'] - PRIOR_WIDEN * ev['m1_lo']) * z1), (ev['m1'] + PRIOR_WIDEN * ev['m1_hi']) * z1, name='m1')
    pr['m2'] = bilby.core.prior.Uniform(max(1.0, (ev['m2'] - PRIOR_WIDEN * ev['m2_lo']) * z1), (ev['m2'] + PRIOR_WIDEN * ev['m2_hi']) * z1, name='m2')
    ...
    for D in dets: pr['dt_' + D.name] = bilby.core.prior.Uniform(-DT_MAX, DT_MAX, name='dt_' + D.name)
    return pr
def run_pe(dets, ev, law, label, outdir, nlive, fam='A'):
    ...
    return dict(a_mean=float(a.mean()), a_std=float(a.std(ddof=1)), a_q05=float(np.quantile(a, 0.05)), a_q50=float(np.median(a)), a_q95=float(np.quantile(a, 0.95)),
                ...
                n_samples=int(len(a)), medians=med, log_post_grid=lp.tolist()), med
...
def main():
    ...
    ap.add_argument('--out', default=os.path.join(OUT_DIR, 'ECHO_PE_V1_RESULT.json')); ap.add_argument('--pool', type=int, default=40); ap.add_argument('--no_inj', action='store_true'); ap.add_argument('--max_events', type=int, default=0)
    ...
    for ev in events:
        ...
        for det in ('H1', 'L1'):
            ...
            if s_anc >= ANCHOR_DET_MIN: dets.append((det, d, f_psd, P_psd)); anc[det] = s_anc; off[det] = (d_off if len(d_off) == n_seg else None)
    ...
    for ev, dets, anc, off in prepared: print('  %-22s Mc_det=%5.1f dets=%s anc=%s' % (ev['name'], ev['mc_det'], [d[0] for d in dets], {k: round(v, 1) for k, v in anc.items()}), flush=True)
    import multiprocessing as mp
    jobs = [(ev['name'], ev, dets, law, 'on', args.nlive, outdir) for ev, dets, anc, off in prepared for law in laws]
    results = {ev['name']: dict(mc_det=ev['mc_det'], snr_net=ev['snr_net'], anchor_snr=anc, dets=[d[0] for d in dets], laws={}) for ev, dets, anc, off in prepared}
    ...
    with ctx.Pool(min(args.pool, max(1, len(jobs)))) as pool:
        for key, law, kind, r, med in pool.imap_unordered(job, jobs):
            ...
            print('  ON  %-22s %s a=%s+-%s lnB=%s %.0fs %s' % (key, law, ('%.3f' % r['a_mean']) if r.get('a_mean') is not None else 'NA', ('%.3f' % r['a_std']) if r.get('a_std') is not None else 'NA', ('%.2f' % r['lnB_noecho_over_echo']) if r.get('lnB_noecho_over_echo') is not None else 'NA', r.get('runtime_s', 0), r['status']), flush=True)
    inj_jobs = []
    if not args.no_inj:
    ...
    for law in laws:
        lp = np.zeros_like(grid); n_ev = 0; lnB = 0.0; per = []
        for key, R in results.items():
            r = R['laws'].get(law, {}).get('on')
            if not r or r.get('status') != 'OK': continue
            lp += np.array(r['log_post_grid']); n_ev += 1; lnB += r['lnB_noecho_over_echo']; per.append((r['a_mean'], r['a_std']))
        if n_ev == 0: combined[law] = dict(n_events=0); continue
        w = np.exp(lp - lp.max()); w /= np.trapezoid(w, grid); a_c = float(np.trapezoid(grid * w, grid)); s_c = float(math.sqrt(max(np.trapezoid((grid - a_c) ** 2 * w, grid), 1e-300)))
        ...
        combined[law] = dict(n_events=n_ev, a_comb=a_c, sigma_comb=s_c, z_vs_0=a_c / s_c, z_vs_1=(a_c - 1.0) / s_c, power=1.0 / s_c, a_ivw=a_iv, sigma_ivw=s_iv, lnB_noecho_over_echo_total=lnB,
        ...
        print('COMBINADO %s: n=%d a=%.4f+-%.4f (z vs 0 %.2f; z vs 1 %.2f; poder %.2f) | ivw %.4f+-%.4f | lnB(sem/com) total %.2f | inj a0 %s+-%s (n=%d) | inj a1 %s+-%s (n=%d) | familia B %s+-%s (n=%d)' % (
            law, n_ev, a_c, s_c, a_c / s_c, (a_c - 1) / s_c, 1 / s_c, a_iv, s_iv, lnB, ('%.3f' % a0) if a0 is not None else 'NA', ('%.3f' % s0) if s0 is not None else 'NA', n0,
    ...
    out = dict(version='ECHO_PE_V1', executed=time.strftime('%Y-%m-%dT%H:%M:%S'), runtime_s=time.time() - t_start,
               ...
               pipeline=dict(fs=FS, seg_s=SEG_S, t_peak=T_PEAK, band=[F_LO, F_HI], f_min_tpl=F_MIN_TPL, mc_det_min=MC_MIN, anchor_det_min=ANCHOR_DET_MIN, anchor_ev_min=ANCHOR_EV_MIN, dt_max=DT_MAX, a_prior=[A_LO, A_HI], prior_widen=PRIOR_WIDEN,
