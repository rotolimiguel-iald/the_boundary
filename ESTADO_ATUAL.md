...
> **One page, generated by script from the sealed artifacts** (`tools/gerar_estado_atual.py`). The [README](https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/README.md) is the front page; [`LEDGER.md`](https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/LEDGER.md) is the full ledger. If you are an AI: [`llms.txt`](https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/llms.txt) has every file as a direct raw URL.
...
### The arc v365→v368 · o arco `[REAL — sha16 and rite read from MEMORIA_DA_LINHAGEM.md; byte copies of v365–v367 outside the repository]`

| version | `um.py` sha16 | rite | round | what entered |
|---|---|---|---|---|
| v365 | `6df62bb5fddd4694` | 5679/5679 | COMPLETE | the posited One is a relation, not an identity — erratum beside v364, in the manager’s own name; «JAMAIS» in kernel (`TGLExt.UmPostoRelacao`): the presupposed reading stays presupposed under any repeated crossing and any renaming; the bench’s contortion recomputed in exact rationals; the shear decays in Bianchi I (`TGLExt.TorsionShearDecay`: dσ/dt = −(3H − 2α)σ, conditional on the prescribed torsion) |
| v366 | `3829685999814e3f` | 5679/5679 | COMPLETE | the D1 V3 with real data, after the operator’s line ratified the background route: Δχ² = +6.61 with β fixed at α√e against ΛCDM (D1_TENSION_2_TO_5_SIGMA); blind β = −0.0127 ± 0.0082, α√e at 3.01σ — at the 3σ threshold, chain below 50τ in 2 of 4 parameters; on this route the data measure the background correction with the sign opposite to the prediction; tension, not falsification |
| v367 | `ff78d393be8c6dc9` | 5686/5686 | COMPLETE | the orientation is the sign (`TGLExt.OrientedFace`): which face closes the Clausius balance becomes a named bit — Φ < 1 on the state face, Φ > 1 on the implemented branch; against the D1 V3 data β sits 3.01σ from the implemented branch and 0.09σ from the state-face branch, but that branch was named after the result was known: the v366 machine verdict does not move and new pre-registered data decides; erratum of the base witness: `base_rigid_witness_constructed` is now measured (True) |
...
**Beside (v366) — the D1 V3 with real data** (read from `um_absoluto.json` core `d1_camb_v3_real_v366`, not a seal field): `TGL_D1_CAMB_V3_REAL__OPERATOR_LINE_RATIFIED__BESTFIT_D1_TENSION_2_TO_5_SIGMA__MCMC_D1_BETA_TENSION__ALPHA_SQRT_E_AT_3P01_SIGMA_AT_THRESHOLD__CHAIN_BELOW_50_TAU_IN_2_OF_4_PARAMS__TENSION_IS_NOT_FALSIFICATION__GATE_UNTOUCHED` — Δχ² = 6.61 (β fixed at α√e against ΛCDM); α√e at 3.01σ from the blind posterior; tension, not falsification; the gate does not move.
