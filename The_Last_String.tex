...
\begin{equation}
d_{\text{crit}} = 9 \quad (95\%\ \text{CI:}\ [7, 16]), \qquad P(9 \leq d_{\text{crit}} \leq 11) = 0.449
\label{eq:dcrit}
\end{equation}
At $d = 9$, $\log_{10}[\alpha^2(9)/\alpha^2(3)] = -6.1$; at $d = 10$, $\log_{10} = -7.1$.
...
Protocol~\#12 is implemented in \texttt{TGL\_GW\_Echo\_Unification\_v1\_4.py} (1,397 lines, Python~3.10+). The dimensional coupling analysis (\S\ref{sec:dimensional}) is implemented in \texttt{TGL\_dimensional\_coupling\_v1.py}. Both codes require PyTorch with CUDA support and run optimally on NVIDIA RTX-class GPUs.
