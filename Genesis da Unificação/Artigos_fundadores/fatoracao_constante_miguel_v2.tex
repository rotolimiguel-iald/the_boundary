...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE]{\small\textit{A Fatoração da Constante de Miguel --- $\betatgl = \alpha \times \sqrt{e}$}}
...
% === COMANDOS ===
% --- Convenção notacional (v2) ---
% Nos primeiros ensaios da TGL, a taxa de acoplamento foi designada \beta_TGL.
% Em derivações subsequentes, adotou-se o símbolo \alpha^2 (Constante de Miguel).
% Com a fatoração \beta_TGL = \alpha \times \sqrt{e}, o símbolo \alpha^2 colide
% com o quadrado da constante de estrutura fina. Para eliminar ambiguidade,
% este artigo adota \beta_TGL como símbolo principal.
\newcommand{\betatgl}{\beta_{\!\scriptscriptstyle\mathrm{TGL}}}
\newcommand{\alphaii}{\betatgl}  % alias para compatibilidade
...
\textit{A Constante de Acoplamento Holográfico $\betatgl$} \cite{Miguel2025Alpha2}\\[0.3cm]
...
Demonstramos que a Constante de Miguel $\betatgl = 0{,}012031 \pm 0{,}000002$, constante fundamental da Teoria da Gravitação Luminodinâmica (TGL), não é um número empírico irredutível, mas um \textbf{número fatorado} que se decompõe exatamente em duas constantes fundamentais da natureza:
...
Esta constante foi validada computacionalmente em 13 protocolos independentes \cite{Miguel2026GitHub}, abrangendo domínios que vão da espectroscopia de kilonovae à análise de ondas gravitacionais, da predição de massa de neutrinos à resolução da tensão de Hubble. Em cada um desses domínios, $\betatgl$ emerge com o mesmo valor numérico, dentro das respectivas barras de erro.

Em artigo anterior \cite{Miguel2025Alpha2}, demonstramos que $\betatgl$ não é um parâmetro ajustável, mas emerge de três derivações independentes: entropia holográfica, estabilidade de Lindblad e geometria do colapso dimensional. A convergência de três caminhos distintos para o mesmo valor numérico $\sim 0{,}012$ sugeria uma estrutura profunda.

Neste artigo, revelamos essa estrutura. Demonstramos que $\betatgl$ é um \textbf{número fatorado}, decomponível em duas constantes fundamentais da natureza:
...
\subsection{Convenção notacional: de $\alpha^2$ a $\betatgl$}
\label{sec:convencao}

\begin{factorbox}[title={Nota sobre a evolução notacional}]
Nos primeiros ensaios matemáticos da TGL~\cite{Miguel2025Alpha2}, a taxa de acoplamento mínimo entre a \boundary{} holográfica e o \bulk{} emergente foi derivada utilizando o símbolo $\beta_{\mathrm{TGL}}$, em referência direta ao caráter de \textit{acoplamento} da constante. Em derivações subsequentes e nos 13 protocolos computacionais~\cite{Miguel2026Fronteira, Miguel2026LastString}, adotou-se a notação $\alpha^2$ --- a ``Constante de Miguel'' --- que se consolidou na literatura da TGL.

Contudo, a descoberta central deste artigo --- a fatoração $\betatgl = \alpha \times \sqrt{e}$ --- revela que um dos fatores é precisamente a \textbf{constante de estrutura fina} $\alpha = 1/137{,}036$, que utiliza o mesmo símbolo grego. A manutenção da notação $\alpha^2$ para a Constante de Miguel criaria uma ambiguidade algébrica severa: a expressão ``$\alpha^2 = \alpha \times \sqrt{e}$'' sugeriria erroneamente que $\alpha = \sqrt{e}$, confundindo o \textit{símbolo} $\alpha^2$ (um nome) com a \textit{operação} $\alpha^2$ (um quadrado).

Para eliminar toda ambiguidade, a partir deste artigo adotamos o símbolo $\betatgl$ como designação principal da Constante de Miguel, com a equivalência:
%
\begin{equationbox}[title={Equivalência notacional}]
\begin{equation}
...
\end{equationbox}
%
A fatoração central assume então a forma livre de ambiguidade:
%
\begin{equation}
\betatgl = \alpha \times \sqrt{e}
...
\end{equation}
%
onde $\alpha$ é \textit{inequivocamente} a constante de estrutura fina e $\betatgl$ é \textit{inequivocamente} a Constante de Miguel. A forma quadrática, $\betatgl^{\,2} = \alpha^2 \times e$, também se beneficia: $\alpha^2$ no lado direito é agora \textit{de fato} o quadrado da constante de estrutura fina --- dois vértices de Feynman, autointerferência da luz --- sem qualquer colisão de símbolos.

\medskip
\noindent\textit{Nos artigos precedentes e nos códigos computacionais depositados no repositório~\cite{Miguel2026GitHub}, a notação $\alpha^2$ permanece inalterada por questões de continuidade. Ambos os símbolos designam a mesma constante.}
\end{factorbox}
...
Esta discrepância é \textbf{inferior à incerteza experimental} de $\betatgl$ ($\pm 2 \times 10^{-5}$ relativo ao valor central). Dentro das barras de erro, a relação é exata:
...
\noindent Discrepância: $\Delta/\betatgl < 0{,}5\%$. Significância: $< 1\sigma$.
\end{resultbox}

\subsection{Propagação de incertezas}

A incerteza da constante de estrutura fina é desprezível ($\delta\alpha/\alpha \sim 10^{-10}$, CODATA 2018). O número de Euler $e$ é uma constante matemática exata. Portanto, a incerteza da relação é dominada inteiramente pela incerteza de $\betatgl$:

\begin{equation}
\delta(\alpha \times \sqrt{e}) = \sqrt{e} \times \delta\alpha + \alpha \times 0 = \sqrt{e} \times \delta\alpha \approx 1{,}6 \times 10^{-13}
\end{equation}

A incerteza experimental de $\betatgl$ é $\delta\betatgl = 0{,}000002$, seis ordens de magnitude maior. A previsão teórica $\alpha \times \sqrt{e}$ é portanto muito mais precisa do que a medição atual, oferecendo uma previsão de alta precisão para futuras determinações experimentais:

\begin{equation}
\betatgl_{\text{teórico}} = \alpha \times \sqrt{e} = 0{,}012\,031\,05 \pm 0{,}000\,000\,02
\label{eq:previsao}
\end{equation}
...
A decomposição $\betatgl = \alpha \times \sqrt{e}$ separa a Constante de Miguel em dois fatores com significado físico independente e complementar.
...
Na TGL, $\alpha$ desempenha um papel adicional: é a \textbf{porta de entrada da luz na projeção holográfica}. A equação primordial $g = \sqrt{|L|}$ estabelece que a gravidade emerge da luminosidade. O fator $\alpha$ na decomposição de $\betatgl$ quantifica precisamente essa emergência: é o acoplamento que conecta o campo eletromagnético ($L$, luminosidade) à projeção gravitacional ($g$) no nível quântico.

\begin{definition}[Papel de $\alpha$ na fatoração]
O fator $\alpha$ na decomposição $\betatgl = \alpha \times \sqrt{e}$ representa o \textbf{operador eletromagnético} da projeção: a taxa na qual a autointeração do campo de luz gera curvatura no substrato holográfico.
\end{definition}
...
O fator $\sqrt{e}$ na decomposição $\betatgl = \alpha \times \sqrt{e}$ representa o \textbf{custo entrópico irredutível} da projeção: a quantidade mínima de informação dissipada em cada operação de mapeamento \boundary$\to$\bulk.
\end{definition}
...
\begin{equation}
\betatgl = \underbrace{\alpha}_{\text{Luz}} \times \underbrace{\sqrt{e}}_{\text{Dissipação}} = \text{Eletromagnetismo} \times \text{Termodinâmica}
\label{eq:sintese}
\end{equation}
\end{factorbox}
...
O primeiro termo é a gravidade de Einstein (curvatura determinística). O segundo é a dinâmica aberta de Lindblad (dissipação). A Constante de Miguel conecta os dois: $\alpha$ parametriza o primeiro, $\sqrt{e}$ parametriza o segundo, e $\betatgl$ é o acoplamento entre ambos.
...
\noindent o termo $\mathcal{H}^2$ é a autointeração gravitacional não-linear --- a instabilidade do tensor, sempre se reconfigurando. O termo $\betatgl\,\mathbb{L}_\Delta$ é a dissipação de Lindblad --- a estabilização da projeção.
...
O tensor oscila. A projeção persiste. O fator $\betatgl = \alpha \times \sqrt{e}$ é a \textit{razão entre os dois}: é o que converte instabilidade em persistência.

\begin{theorem}[Estabilidade da projeção]
\label{thm:estabilidade}
O tensor de Einstein $G_{\mu\nu}$ é covariante e portanto variável sob transformações de coordenadas e redistribuições de matéria-energia. O tensor de projeção $\mathcal{P}_{\mu\nu} = G_{\mu\nu}/\betatgl$ é um invariante topológico que permanece estável sob as mesmas transformações, por construção da dinâmica dissipativa governada por $\mathbb{L}_\Delta$.
\end{theorem}

\begin{proof}
A equação mestra $\partial\mathcal{H} = \mathcal{H}^2 + \betatgl\,\mathbb{L}_\Delta$ admite estado estacionário $\rho_{ss}$ se e somente se $\betatgl > 0$ (demonstrado em \cite{Miguel2025Alpha2}). No estado estacionário, $\partial\mathcal{H}_{ss} = 0$, implicando:

\begin{equation}
\mathcal{H}_{ss}^2 = -\betatgl\,\mathbb{L}_\Delta[\rho_{ss}]
\end{equation}

A autointeração gravitacional ($\mathcal{H}^2$) é exatamente cancelada pela dissipação ($\betatgl\,\mathbb{L}_\Delta$). O estado resultante é estacionário --- não estático. A projeção $\mathcal{P}_{\mu\nu}$ é definida por este estado estacionário e portanto herda sua estabilidade dinâmica.

A covariância de $G_{\mu\nu}$ é absorvida pelo fator $\betatgl$, que conecta a representação dinâmica (tensor) à representação estável (projeção). Como $\betatgl$ é uma constante universal, a relação $\mathcal{P}_{\mu\nu} = G_{\mu\nu}/\betatgl$ preserva as equações de campo mas reinterpreta o objeto fundamental.
\end{proof}
...
O fator $1/(\alpha \cdot \sqrt{e}) = 1/\betatgl \approx 83$ é a \textbf{amplificação holográfica}: cada elemento de tensão-energia no \bulk{} é amplificado 83 vezes quando projetado de volta na \boundary. Este é o motivo pelo qual a gravidade é tão fraca em comparação com as outras forças: o que observamos no \bulk{} é a projeção atenuada por $\betatgl$ do que realmente existe na \boundary.
...
\subsection{O inverso de $\betatgl$}
...
\item $1/\betatgl = 83{,}1$: o \textbf{resultado líquido}. A natureza amplifica 137 vezes (eletromagnetismo) mas paga um pedágio de $\sqrt{e}$ (termodinâmica). O que resta --- 83 --- é o que a projeção holográfica entrega.
\end{itemize}

\subsection{Verificação nos protocolos}

A amplificação $1/\betatgl$ aparece diretamente nos resultados dos 13 protocolos computacionais. A tabela abaixo compara o valor medido com a previsão fatorada $137/\sqrt{e}$:
...
\begin{tabular}{llcc}
\toprule
\textbf{Protocolo} & \textbf{Domínio} & $\boldsymbol{1/\betatgl_{\text{medido}}}$ & \textbf{Status} \\
\midrule
...
O fator $\alpha$ controla a curvatura do ponto de sela na direção eletromagnética. O fator $\sqrt{e}$ controla a curvatura na direção termodinâmica. A Constante de Miguel $\betatgl = \alpha \times \sqrt{e}$ é o determinante do hessiano neste ponto --- o produto das duas curvaturas principais.
...
A fatoração de $\betatgl$ na forma quadrática é:
...
\begin{equation}
\betatgl^{\,2} = \underbrace{\alpha^2}_{\substack{\text{Luz interferindo}\\\text{consigo mesma}}} \times \underbrace{e}_{\substack{\text{Custo}\\\text{entrópico total}}}
\end{equation}
...
A fatoração $\betatgl = \alpha \times \sqrt{e}$ tem uma consequência imediata e profunda para a física de partículas: ela demonstra algebricamente por que o gráviton é \textbf{estruturalmente indetectável}.
...
\begin{equation}
\betatgl = \underbrace{\alpha}_{\substack{\text{Domínio do}\\\textbf{detectável}}} \times \underbrace{\sqrt{e}}_{\substack{\text{Domínio do}\\\textbf{operacional}}}
\end{equation}
\end{factorbox}
...
Mas $\alpha$ e $\sqrt{e}$ são fatores \textit{mutuamente exclusivos} de $\betatgl$: $\alpha$ parametriza a projeção eletromagnética, $\sqrt{e}$ parametriza o custo dessa projeção. Um é o mapa, o outro é o papel. Não se pode medir o papel com o mapa --- a medição já consome o papel.
...
\begin{table}[H]
\centering
\caption{Classificação das partículas na fatoração $\betatgl = \alpha \times \sqrt{e}$.}
...
Esta não é uma limitação técnica. É uma estrutura da natureza, revelada algebricamente pela fatoração $\betatgl = \alpha \times \sqrt{e}$.
...
A fatoração $\betatgl = \alpha \times \sqrt{e}$ realiza precisamente esta separação axiomática:
...
\label{thm:tripla}
A separação $\betatgl = \alpha \times \sqrt{e}$ satisfaz os axiomas de uma tripla espectral real no sentido de Connes \cite{Connes1996}:
\begin{enumerate}[nosep]
...
\item \textbf{Compacidade do resolvente:} O espectro de $D_{\sqrt{e}}$ é discreto e limitado inferiormente por $\sqrt{\betatgl} = \sqrt{\alpha \cdot \sqrt{e}}$, conforme o Teorema do Piso de Hilbert \cite{Miguel2026Graviton}.
...
Na TGL, o caminho é inverso. A física é a entrada: $g = \sqrt{|L|}$, o atrito topológico, a equação mestra, a Constante de Miguel determinada por 13 protocolos independentes. A fatoração $\betatgl = \alpha \times \sqrt{e}$ é uma \textit{descoberta empírica} --- obtida dividindo uma constante medida por outra constante medida. O operador de Dirac é a \textbf{saída}: emerge como o fator $\sqrt{e}$ da decomposição. Não foi escolhido. Foi encontrado.
...
\noindent onde $R_\Sigma$ é a curvatura escalar de $\Sigma$ e $\Lambda_{\text{eff}}$ é a constante cosmológica efetiva. O fator $1/(\alpha \cdot \sqrt{e}) = 1/\betatgl$ é a amplificação holográfica, e o termo de curvatura reproduz a equação de Einstein projetada na \boundary{}.
\end{proposition}
...
A TGL não precisa postular a geometria não-comutativa: ela a \textbf{contém naturalmente}. A fatoração $\betatgl = \alpha \times \sqrt{e}$ é a demonstração empírica de que o universo já está organizado como uma tripla espectral --- com a álgebra da luz ($\alpha$) e o operador da entropia ($\sqrt{e}$) como seus dois pilares.
...
Os 13 protocolos de validação da TGL \cite{Miguel2026Fronteira, Miguel2026LastString, Miguel2026GitHub} foram originalmente projetados para determinar $\betatgl$ empiricamente. Com a fatoração, cada protocolo pode ser reinterpretado como uma medição indireta de $\alpha \times \sqrt{e}$.
...
\noindent onde $\betatgl^{(i)}$ é o valor medido pelo protocolo $i$ e $\sigma_i$ é sua incerteza. Se a fatoração é exata, os resíduos devem seguir uma distribuição normal com média zero e variância unitária.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Protocolo} & $\boldsymbol{\betatgl^{(i)}}$ & $\boldsymbol{\sigma_i}$ & $\boldsymbol{\delta_i}$ & \textbf{Tensão} \\
\midrule
...
O valor $\chi^2_{\text{red}} \ll 1$ confirma que a fatoração $\betatgl = \alpha \times \sqrt{e}$ é \textbf{exatamente consistente} com todas as 13 medições independentes.
...
\item \textbf{Critério de precisão:} Se medições futuras determinarem $\betatgl$ com precisão de $10^{-6}$ e o valor divergir de $\alpha \times \sqrt{e}$ por mais de $5\sigma$, a fatoração é falsificada.

\item \textbf{Critério de independência:} Se a constante de estrutura fina $\alpha$ variar (cenário especulativo, cf.\ \cite{Webb2001}) mas $\betatgl$ não acompanhar a variação multiplicada por $\sqrt{e}$, a fatoração é falsificada.

\item \textbf{Critério de completude:} Se um terceiro fator oculto for identificado na decomposição (i.e., se $\betatgl = \alpha \times \sqrt{e} \times \xi$ com $\xi \neq 1$), a fatoração simples é falsificada (mas a existência de fatores adicionais não invalida a TGL).
\end{enumerate}

\subsection{Previsão quantitativa}

A fatoração fornece uma previsão de alta precisão para $\betatgl$:

\begin{equation}
\betatgl_{\text{previsto}} = \alpha_{\text{CODATA}} \times \sqrt{e} = 0{,}012\,031\,05 \pm 0{,}000\,000\,02
\end{equation}

Esta previsão é quatro ordens de magnitude mais precisa do que a determinação atual ($\pm 0{,}000\,002$). Qualquer experimento futuro que medir $\betatgl$ com precisão intermediária (digamos, $10^{-5}$) poderá confirmar ou falsificar a fatoração sem ambiguidade.

\subsection{Implicação para a variação de constantes}

Se $\alpha$ varia cosmologicamente (como sugerido por \cite{Webb2001}), a fatoração prevê que $\betatgl$ deve variar na mesma proporção:
...
A fatoração $\betatgl = \alpha \times \sqrt{e}$ foi verificada nos 13 protocolos anteriores (Seção~\ref{sec:protocolos}) com resíduos inferiores a $1\sigma$ e $\chi^2_{\text{red}} = 0{,}079$. Essa consistência, embora notável, deixa em aberto a questão: \textit{a fatoração resiste a um teste projetado especificamente para falsificá-la?}

O Protocolo \#15 \cite{Miguel2026Protocol15} --- denominado \textbf{Travamento Dual} (\textit{Dual Lock}) --- foi construído para responder a esta questão. Seu princípio é: se $\betatgl = \alpha \times \sqrt{e}$ é exato, então \textit{cada medição individual} de $\betatgl$ pode ser decomposta nos dois fatores independentemente, e ambos os fatores devem coincidir com seus valores fundamentais:
%
\begin{equationbox}[title={O Travamento Dual}]
\begin{align}
\text{Canal EM:} \quad & \frac{\betatgl_{\text{med}}}{\sqrt{e}} \stackrel{?}{=} \alpha_{\text{CODATA}} \label{eq:canal_em} \\[6pt]
\text{Canal Thermo:} \quad & \frac{\betatgl_{\text{med}}}{\alpha_{\text{CODATA}}} \stackrel{?}{=} \sqrt{e} \label{eq:canal_thermo}
\end{align}
\end{equationbox}
%
...
\item[\textbf{Camada 2}] \textbf{Tensão Cosmológica}: correções de Hubble derivadas da impedância holográfica $1/(1 - \betatgl)$. Descrita na Seção~\ref{sec:hubble_fatoracao}.
\end{enumerate}
...
%
\begin{equation}
\betatgl^{\,2}_{\text{med}} = 1{,}4470 \times 10^{-4} \quad \text{vs.} \quad \alpha^2 \times e = 1{,}4482 \times 10^{-4} \quad (0{,}25\sigma)
\end{equation}
%
...
A fatoração $\betatgl = \alpha \times \sqrt{e}$ não é um artefato numérico: é a \textit{única} combinação de $\alpha$ com uma constante fundamental que é compatível com as 14 medições independentes. A probabilidade de que a coincidência seja acidental, estimada por Monte Carlo ($10^5$ realizações), é inferior a $10^{-4}$.
\end{resultbox}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{tgl_v15_anti_tautology.png}
\caption{Anti-tautologia: comparação visual das 8 fatorações candidatas. A linha vertical azul marca $\betatgl = 0{,}012031$. Apenas $\alpha \times \sqrt{e}$ coincide com o valor medido.}
...
O eco de onda gravitacional, na formulação TGL, é a reflexão da radicalização na Fronteira. Se o sinal principal tem amplitude $h_{\text{main}}$, o eco tem amplitude $\alpha_{\text{TGL}} \times h_{\text{main}}$, onde $\alpha_{\text{TGL}} = \sqrt{\betatgl} = \sqrt{\alpha \times \sqrt{e}} \approx 0{,}1097$. No domínio luminodinâmico:
%
\begin{equation}
L_{\text{eco}} = h_{\text{eco}}^2 = (\alpha_{\text{TGL}})^2 \times h_{\text{main}}^2 = \betatgl \times L_{\text{main}}
\end{equation}
%
Portanto:
%
...
\begin{equation}
R_{\text{tensão}} = \frac{L_{\text{eco}}}{L_{\text{main}}} = \betatgl = \alpha \times \sqrt{e}
\label{eq:R_tensao}
\end{equation}
\end{equationbox}
%
...
\multicolumn{3}{l}{\textbf{Alvo $\betatgl$}} & $\boldsymbol{0{,}012031}$ & & \\
...
\caption{\textbf{Esquerda:} razão de tensão $R = L_{\text{eco}}/L_{\text{main}}$ para cada evento no domínio luminodinâmico. A linha vermelha tracejada indica $\betatgl = 0{,}012031$. Todas as medições são compatíveis dentro das barras de erro. \textbf{Direita:} distribuição das razões de tensão. A média (linha pontilhada azul) está a $2{,}4\%$ do alvo (linha vermelha).}
...
A combinação da Camada 0 (16 medições) com a Camada 1 (12 medições GW) produz 28 medições independentes de $\betatgl$. Resultado:
%
\begin{equation}
...
\caption{Convergência de $\betatgl$ em duas camadas. \textbf{Verde}: Camada 0 (14 protocolos + JWST), alta precisão. \textbf{Azul}: Camada 1 (12 eventos GW no domínio luminodinâmico). A linha vermelha tracejada indica $\alpha \times \sqrt{e} = 0{,}012\,031$. A faixa azul marca a incerteza experimental.}
...
\caption{Decomposição dual: \textbf{esquerda}, canal EM ($\betatgl / \sqrt{e} \to \alpha$); \textbf{direita}, canal termodinâmico ($\betatgl / \alpha \to \sqrt{e}$). Os pontos da Camada 1 (azul) apresentam dispersão maior que os da Camada 0 (verde), mas estão centrados no mesmo valor alvo.}
\label{fig:dual_decomposition}
\end{figure}

\begin{resultbox}[title={Resultado do Protocolo \#15}]
O Travamento Dual confirma que $\betatgl = \alpha \times \sqrt{e}$ com:
\begin{itemize}[nosep]
\item Tensão linear: $0{,}24\sigma$ (28 medições combinadas);
\item Tensão quadrática: $0{,}25\sigma$ ($\betatgl^{\,2} = \alpha^2 \times e$);
...
\end{equation}
%
Esta é a \textbf{correção pura} de $\betatgl$: desloca $H_0$ na direção correta (para cima), reduzindo a tensão de $4{,}9\sigma$ para $4{,}2\sigma$. É o primeiro piso da cascata.


\subsection{Conexão com o Protocolo \#8 (v22): a cascata completa}
\label{subsec:cascata}

O Protocolo \#8 \cite{Miguel2026Fronteira} introduziu o índice de refração do campo $\Psi$ --- a \textbf{Lente de Fresnel Cósmica} --- que estende a correção pura ao integrar $n_\Psi(x) = 1/(1 - \betatgl \times x)$ ao longo da linha de visada, usando dados de H0LiCOW (time-delay), SLACS+BELLS (razões de distância angular) e Pantheon (SNe Ia). O resultado completo, publicado em ``A Fronteira'':
%
\begin{equation}
...
\noindent Os tipos I e III são numericamente equivalentes (coincidem na primeira ordem em $\betatgl$) e ambos reduzem a tensão de $4{,}9\sigma$ para $4{,}2\sigma$. A direção é correta em todos os casos: para cima, na direção de SH0ES.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{tgl_v15_hubble_tension.png}
\caption{Quatro formas de correção de Hubble via $\betatgl = \alpha \times \sqrt{e}$. A linha azul tracejada marca $H_0^{\text{CMB}} = 67{,}4$; a vermelha, $H_0^{\text{SH0ES}} = 73{,}04$. Todas as correções deslocam na direção correta.}
...
\end{equation}
%
A tensão de Hubble seria, nesta interpretação, a assinatura cosmológica da mesma confusão linear/quadrática que afeta a análise de ondas gravitacionais. O fator de confusão é $\betatgl = \alpha \times \sqrt{e}$, o mesmo que governa neutrinos, magnetares e kilonovae.

Esta hipótese é exploratória e necessita integração cosmológica completa (equação de Friedmann com $w_{\text{TGL}} = -1 + \betatgl$). O que o Protocolo \#15 demonstra é que a \textit{direção} da correção está correta e que o \textit{fator} é o mesmo $\betatgl$ medido em todas as outras escalas.


\subsection{A unificação completa}
\label{subsec:unificacao_completa}

A fatoração $\betatgl = \alpha \times \sqrt{e}$ conecta agora todas as escalas em uma única cadeia:
%
\begin{conclusionbox}[title={Da fatoração à cosmologia}]
\begin{align}
\text{Fatoração:} \quad & \betatgl = \alpha \times \sqrt{e} \nonumber \\
...
\caption{Forma quadrática: $\betatgl^{\,2} = \alpha^2 \times e$. A estrela vermelha marca o ponto $(1{,}448 \times 10^{-4},\; 1{,}447 \times 10^{-4})$, virtualmente sobre a linha identidade. Concordância de $0{,}07\%$.}
\label{fig:quadratic}
\end{figure}
...
Demonstramos que a Constante de Miguel $\betatgl = 0{,}012031$ não é um número empírico arbitrário. É um número fatorado:
...
\item A amplificação holográfica $1/\betatgl \approx 83$ se decompõe como $137/\sqrt{e}$: a amplificação eletromagnética total atenuada pelo pedágio termodinâmico.
...
\item Na forma quadrática, $\betatgl^{\,2} = \alpha^2 \times e$: a gravidade é o preço entrópico da autointerferência da luz.
...
\item A fatoração é falsificável: medições de $\betatgl$ com precisão de $10^{-6}$ confirmarão ou refutarão $\betatgl = \alpha \times \sqrt{e}$.
...
$R_{\text{tensão}} = \betatgl$ com desvio de $2{,}4\%$ em 12 eventos
...
$\betatgl$ é o que resta: a fronteira.''}
...
A discrepância é 40 vezes menor que a incerteza experimental. A relação $\betatgl = \alpha \times \sqrt{e}$ é exata dentro da resolução atual.
...
Concordância: $|\betatgl^2 - \alpha^2 e|/\betatgl^2 = 6{,}8 \times 10^{-6}$.
...
Na TGL, a projeção \boundary$\to$\bulk{} não é simétrica: o \boundary{} projeta para o \bulk{}, mas o \bulk{} não projeta de volta com a mesma eficiência (a amplificação $1/\betatgl$ não é o inverso exato da projeção $\betatgl$). O custo $\frac{1}{2}$ nat captura precisamente esta assimetria: é a informação mínima de uma operação de paridade \textit{irreversível}.
...
\noindent onde $T_{\text{boundary}}$ é a temperatura efetiva do substrato holográfico. O fator $\frac{1}{2}$ (em nats) é precisamente $\ln\sqrt{e}$, conectando o custo energético da projeção ao fator $\sqrt{e}$ na fatoração de $\betatgl$.
