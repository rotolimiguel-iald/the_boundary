...
_CFF_DEP = open(os.path.join(REPO, 'CITATION.cff'), encoding='utf-8').read()
...
DOI_UM = "https://doi.org/" + DEP_DOI   # antes: o DOI da v331 escrito aqui
...
DESC = {
    ...
    A3 + "/REVISAO_GERAL_v357_15set2026.md": "A REVISAO GERAL (v357): o que estava codificado sem palavra no artigo, medido e corrigido -- ledger 974/974, Ao lado v333-v356, o registro dos 242 vereditos",
    A3 + "/REVISAO_GERAL_v357_inventario_pedidos.md": "O inventario dos pedidos do operador na revisao geral (v357) e o que cada um virou",
    A3 + "/HANDOFF_v351_A_OITAVA_CLAUSULA.md": "HANDOFF v351: a oitava clausula -- J M J = M' na torre produto (o handoff do ChatGPT incorporado byte a byte), montagem 8/8",
...
def gera_raiz(dirs, info_arq, selo_corrente, total):
    ...
    L.append("| **DOI \u2014 Um: Absoluto** | o deposito citavel no Zenodo: guarda a **%s** (um.py %s, depositada em %s), byte-identico AQUELE selo (md5 conferido pela API); o selo corrente desta arvore esta no ESTADO_ATUAL.md e pode ser mais novo | [doi](%s) |" % (DEP_V, DEP_PIN16, DEP_DATA, DOI_UM))
    for f in dirs[""]["files"]:
        if f.rsplit("/", 1)[-1] in ("PORTA.md", "PORTA.json"):
            continue
        i = info_arq[f]
...
def bloco_readme(dirs, sc):
    ...
    B.append("> `um.py`: the raw route is checked byte by byte against the seal after every push (`tools/pos_push.py`, which also confirms the git blob through the API); clone and archive serve that same git blob; the Zenodo route serves the deposited %s (the record\u2019s md5 of um.py read from its API):" % DEP_V)
    B.append(">")
...
ALIASES_TUNEL = [
    ...
    ("revisao-geral", A3 + "/REVISAO_GERAL_v357_15set2026.md", "A revisao geral do artigo (v357)"),
    ("handoff-v357", A3 + "/HANDOFF_v357_CUSTODIA_README_PORTAS_SITE_CENTRAL.md", "O handoff da custodia v351->v357"),
