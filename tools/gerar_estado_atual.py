...
def main() -> int:
    ...
    L.append("> **One page, generated by script from the sealed artifacts** (`tools/gerar_estado_atual.py`). "
             "The [README](%s) is the front page; [`LEDGER.md`](%s) is the full ledger. If you are an AI: [`llms.txt`](%s) has every file as a direct raw URL." % (raw("README.md"), raw("LEDGER.md"), raw("llms.txt")))
    L.append("")
    ...
    # custódia v368 (19/09/2026, sessão da Central por ordem do operador): o arco v365->v368, ao lado; sha16 e rito LIDOS do diário
    arco368 = {}
    for mm in re.finditer(r"^## [^\n]*\*\*(v36[5-8]) SELADA[^*\n]*\*\*[^\n]*\n", diario, re.M):
        fim = diario.find("\n## ", mm.end())
        bl = diario[mm.end(): fim if fim > 0 else len(diario)]
        sh = re.findall(r"`um\.py` sha16 `([0-9a-f]{16})`", bl)
        ri = re.findall(r"\*\*(\d+)/(\d+)\*\*", bl)
        arco368[mm.group(1)] = (sh[-1] if sh else "?", ("%s/%s" % ri[-1]) if ri else "?")
    if arco368:
        L.append("")
        L.append("### The arc v365\u2192v368 \u00b7 o arco `[REAL \u2014 sha16 and rite read from MEMORIA_DA_LINHAGEM.md; byte copies of v365\u2013v367 outside the repository]`")
        L.append("")
        L.append("| version | `um.py` sha16 | rite | round | what entered |")
        L.append("|---|---|---|---|---|")
        for v, rnd, txt in [('v365', 'COMPLETE', 'the posited One is a relation, not an identity — erratum beside v364, in the manager’s own name; «JAMAIS» in kernel (`TGLExt.UmPostoRelacao`): the presupposed reading stays presupposed under any repeated crossing and any renaming; the bench’s contortion recomputed in exact rationals; the shear decays in Bianchi I (`TGLExt.TorsionShearDecay`: dσ/dt = −(3H − 2α)σ, conditional on the prescribed torsion)'), ('v366', 'COMPLETE', 'the D1 V3 with real data, after the operator’s line ratified the background route: Δχ² = +6.61 with β fixed at α√e against ΛCDM (D1_TENSION_2_TO_5_SIGMA); blind β = −0.0127 ± 0.0082, α√e at 3.01σ — at the 3σ threshold, chain below 50τ in 2 of 4 parameters; on this route the data measure the background correction with the sign opposite to the prediction; tension, not falsification'), ('v367', 'COMPLETE', 'the orientation is the sign (`TGLExt.OrientedFace`): which face closes the Clausius balance becomes a named bit — Φ < 1 on the state face, Φ > 1 on the implemented branch; against the D1 V3 data β sits 3.01σ from the implemented branch and 0.09σ from the state-face branch, but that branch was named after the result was known: the v366 machine verdict does not move and new pre-registered data decides; erratum of the base witness: `base_rigid_witness_constructed` is now measured (True)'), ('v368', 'COMPLETE', 'the inversion of the Name as the operator’s decision [INPUT/ONTO] (`TGLExt.NameIsTheContent`: the Name, the content, determines the whole inscription; the form alone does not; 1 = 1 is `rfl`; 1 = 0 in a ring collapses everything to zero) — erratum of the manager beside: the earlier composition chained a homonym and falls; the spinorial obstruction (`TGLExt.SpinorObstruction`): no intertwining linear projection takes integer spin to spin ½ — the electron sector is not constructed [OPEN] and the sentence «the electron is the shadow of the graviton in the bulk» stays [ONTO]')]:
            sh, ri = arco368.get(v, ("?", "?"))
            L.append("| %s | `%s` | %s | %s | %s |" % (v, sh, ri, rnd, txt))
        L.append("")
    ...
    if nat:
        ...
        # custodia v368 (19/09): a V3 do D1 com dado real (v366) -- nao e' campo do selo; lida do mundo (core.d1_camb_v3_real_v366), AO LADO da tabela
        _d1 = ((mundo_json.get("core") or {}).get("d1_camb_v3_real_v366") or {})
        if _d1.get("verdict"):
            assert "CONFIRMED" not in str(_d1["verdict"]), "veredito CONFIRMED no mundo -- PARAR"
            L.append("")
            L.append("**Beside (v366) \u2014 the D1 V3 with real data** (read from `um_absoluto.json` core `d1_camb_v3_real_v366`, not a seal field): `%s` \u2014 \u0394\u03c7\u00b2 = %.2f (\u03b2 fixed at \u03b1\u221ae against \u039bCDM); \u03b1\u221ae at %.2f\u03c3 from the blind posterior; tension, not falsification; the gate does not move." % (_d1["verdict"], float(_d1.get("delta_chi2")), float(_d1.get("z_alpha_sqrt_e"))))
        L.append("")
    ...
    def _kb(rel):
        if rel == "ESTADO_ATUAL.md":
            return "__KB_DESTA_PAGINA__"   # errata 19/09 (v368): medido depois de escrita (antes: "about 12 KB", fixo)
    ...
    _i = [j for j, l in enumerate(L) if "__KB_DESTA_PAGINA__" in l]
    if _i:   # o tamanho desta pagina, medido depois de montada
        _base = L[_i[0]]
        L[_i[0]] = _base.replace("__KB_DESTA_PAGINA__", "0 KB")
        _t = "\n".join(L)   # write_text no Windows grava CRLF: conta 1 byte a mais por quebra de linha
        L[_i[0]] = _base.replace("__KB_DESTA_PAGINA__", "%.0f KB" % ((len(_t.encode("utf-8")) + (_t.count("\n") if sys.platform.startswith("win") else 0)) / 1024.0))
