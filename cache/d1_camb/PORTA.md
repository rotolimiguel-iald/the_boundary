# PORTA -- `cache/d1_camb`

porta acima: https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/cache/PORTA.md

> **A REGRA DA PORTA.** Toda pasta canonica tem `PORTA.md` + `PORTA.json`;
> toda porta aponta para cima e para baixo. Todo link abaixo e' a URL raw
> DIRETA do arquivo -- nao ha nome de pasta para adivinhar.

Pasta do repositorio the_boundary com 3 arquivo(s) rastreado(s).

## A PORTA ACIMA

| destino | link |
|---|---|
| PORTA.md da pasta acima (`cache`) | https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/cache/PORTA.md |
| PORTA.json da pasta acima | https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/cache/PORTA.json |
| PORTA.md da RAIZ | https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/PORTA.md |
| `llms.txt` (a porta de entrada para IA) | https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/llms.txt |
| `README.md` (o atlas da fronteira) | https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/README.md |
| o site | https://teoriadagravitacaoluminodinamica.com |
| o repositorio | https://github.com/rotolimiguel-iald/the_boundary |

## OS ARQUIVOS DESTA PASTA

3 arquivo(s) -- pasta no GitHub: https://github.com/rotolimiguel-iald/the_boundary/tree/main/cache/d1_camb

**RESULTADO SELADO**

| arquivo | papel | link raw direto |
|---|---|---|
| `D1_CAMB_DIAGNOSTICO.json` | Diagnostico do bug de integracao de distancia do worker CAMB de maio (+20% com beta = 0) (v349) | [raw](https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/cache/d1_camb/D1_CAMB_DIAGNOSTICO.json) |
| `D1_CAMB_RESULT.json` | RESULTADO lido por hash: D1 via CAMB V1 (script de maio intocado) -- expos o bug de distancia do worker; autopsia por hash (v349) | [raw](https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/cache/d1_camb/D1_CAMB_RESULT.json) |
| `D1_CAMB_V2_RESULT.json` | RESULTADO lido por hash: D1 via CAMB V2 (worker corrigido) -- Delta chi2 9,70 (D1_TENSION_2_TO_5_SIGMA); beta livre -0,0171 +- 0,0075 (v349) | [raw](https://raw.githubusercontent.com/rotolimiguel-iald/the_boundary/main/cache/d1_camb/D1_CAMB_V2_RESULT.json) |

---

gerado por script de git ls-files em 2026-09-15 -- nao editar a mao
